...
import aboutMeImage from "../../../../assets/image/mypics/me2.jpg";
                ...
                <div className="flex flex-col justify-center items-center row-start-2 sm:row-start-1">
                    <h1 className="leading-[70px] mb-4 text-3xl lg:text-4xl xl:text-5xl font-medium">
                        {headingText.split(" ").map((item, index) => (
                            <span
                                className={`${index === 6 ? "text-red-main" : "text-[#000]"}`}
                                key={index}
                            >
                                {item}{" "}
                            </span>
                ...
                <AnimationWrapper className={"flex w-full p-4"}>
                    ...
                    >
                        {(data?.skills || "").split(",").map((skill, index) => (
                            ...
                            >
                                <div
                                    className="whitespace-nowrap text-ellipsis overflow-hidden py-3 w-[160px] px-6 border-[2px] border-red-main bg-[#fff] text-[#000] font-semibold rounded-lg text-xl tracking-widest hover:shadow-green-main transition-all outline-none"
                                    title={skill}
                                >
                                    {skill}
                                </div>
